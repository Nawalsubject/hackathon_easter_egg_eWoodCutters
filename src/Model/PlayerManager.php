...
class PlayerManager extends AbstractManager
{
    ...
    public function updateXY(int $x, int $y, int $playerid)
    {
        ...
        // prepared request
        if ($playerid == 1) {
            $statement = $this->pdo->prepare("UPDATE $this->table SET `x_init` = :x, `x_init` = :x, player1_reveal = 1 WHERE id=:id");
        } else {
            $statement = $this->pdo->prepare("UPDATE $this->table SET `x_init` = :x, `x_init` = :x, player2_reveal = 1 WHERE id=:id");
        }
